...
@push('customJs')
    ...
    <!-- Blog Category Delete Multiple -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var datatable = BlogCategoryDataTable.getDataTable();
        ...
        $('#my_delete_items').on('click', function() {
            ...
            swalInit.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '{{ route('backend.blogcategory.deleteMultiple') }}',
                        method: 'POST',
                        data: {
                            '_token': '{{ csrf_token() }}',
                            'ids': selectedIds,
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form class="form-horizontal" id="my_update_item_form"
                    action="{{ route('backend.blogcategory.update') }}" method="POST">
                    @csrf
                    <div class="modal-body">
